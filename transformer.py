...
class Transformer:
	...
	def __init__(self, sess, voca_size, embedding_size, is_embedding_scale, PE_sequence_length,
					encoder_decoder_stack, multihead_num, pad_idx, label_smoothing):
		...
		# [self.PE_sequence_length, self.embedding_siz] #slice해서 쓰자.
		self.PE = tf.convert_to_tensor(self.positional_encoding(), dtype=tf.float32)

		with tf.name_scope("placeholder"):
			# self.lr = tf.placeholder(tf.float32)
			self.encoder_input = tf.placeholder(
			    tf.int32, [None, None, 494], name='encoder_input')
			self.encoder_input_length = tf.shape(self.encoder_input)[1]

			self.decoder_input = tf.placeholder(
			    tf.int32, [None, None], name='decoder_input')  # 'go a b c eos pad'
		...
		with tf.name_scope("embedding_table"):
			with tf.device('/cpu:0'):
				...
				front, end = tf.split(
				    embedding_table, [self.pad_idx, self.voca_size-1-self.pad_idx])
				# [self.voca_size, self.embedding_size]
				self.embedding_table = tf.concat((front, zero, end), axis=0)

		with tf.name_scope('encoder'):
			
			self.encoder_embedding = self.encoder(self.encoder_input)

		with tf.name_scope('decoder'):
			decoder_input_embedding, decoder_input_mask = self.embedding_and_PE(
			    self.decoder_input, self.decoder_input_length)  # decoder_input은 go 붙어있어야함.
			self.decoder_embedding, self.decoder_pred = self.decoder(
			    decoder_input_embedding, self.encoder_embedding, decoder_input_mask)
	...
	def encoder(self, encoder_input_embedding):
		...
		encoder_input_embedding = tf.layers.dense(encoder_input_embedding,self.embedding_size,activation=tf.nn.relu)
		encoder_input_embedding = tf.cast(encoder_input_embedding, dtype=tf.float32)
	...
	def decoder(self, decoder_input_embedding, encoder_embedding, decoder_input_mask):
		...
		pad_of_encoder_embedding = tf.transpose(
				tf.reduce_sum(tf.abs(encoder_embedding), axis=-1, keepdims=True), # [N, self.encoder_input_length, 1]
				[0, 2, 1] 
	...
	def multi_head_attention_add_norm(self, query, key_value, score_mask=None, output_mask=None, activation=None, name=None):
		...
		with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
			...
			softmax = tf.nn.softmax(score, axis=2) # [self.multihead_num*N, query_sequence_length, key_value_sequence_length]
	...
	def run(self,encoder_input,decoder_input):
		with self.sess as sess:
			result = sess.run([self.decoder_embedding], feed_dict={self.encoder_input: encoder_input, \
                self.decoder_input: decoder_input, self.keep_prob:0.8}) 
			return(result[0])


def main():
    
    model = Transformer(sess=tf.Session(), voca_size=2752, embedding_size=256, is_embedding_scale=True, PE_sequence_length=128,
                encoder_decoder_stack=6, multihead_num=8, pad_idx=0, label_smoothing=0.1)

    result = model.run(np.ones([1,1000,494]),np.array([[1,20,300,2000]]))
    print(result)
...
### input: X - [batch,time,features],  Y - [batch,time]
