...
from util.audio_new import audiofile_to_linear_input_vector
...
tf.app.flags.DEFINE_string  ('train_files',      '/home/zhouxinpei/deepspeech/DeepSpeech-0.1.1/bin/4500h_train_data_valid_vocab.csv',          'comma separated list of files specifying the dataset used for training. multiple files will get merged')
tf.app.flags.DEFINE_string  ('dev_files',        '/home/zhouxinpei/deepspeech/DeepSpeech-0.1.1/bin/4500h_dev_data_valid_vocab.csv',          'comma separated list of files specifying the dataset used for validation. multiple files will get merged')
tf.app.flags.DEFINE_string  ('test_files',       '/home/zhouxinpei/deepspeech/DeepSpeech-0.1.1/bin/4500h_dev_data_valid_vocab.csv',          'comma separated list of files specifying the dataset used for testing. multiple files will get merged')
...
tf.app.flags.DEFINE_integer ('epoch',            7,          'target epoch to train - if negative, the absolute number of additional epochs will be trained')
...
tf.app.flags.DEFINE_float   ('learning_rate',    0.0005,       'learning rate of Adam optimizer')

# Batch sizes

tf.app.flags.DEFINE_integer ('train_batch_size', 64,           'number of elements in a training batch')
tf.app.flags.DEFINE_integer ('dev_batch_size',   32,           'number of elements in a validation batch')
tf.app.flags.DEFINE_integer ('test_batch_size',  32,           'number of elements in a test batch')
...
tf.app.flags.DEFINE_integer ('validation_step',  1,           'number of epochs we cycle through before validating the model - a detailed progress report is dependent on "--display_step" - 0 means no validation steps')

# Checkpointing

tf.app.flags.DEFINE_string  ('checkpoint_dir',   '/home/zhouxinpei/deepspeech/DeepSpeech-0.1.1/data/chinese/test_linear_new_20190213/ckpt_0.12_v2/',          'directory in which checkpoints are stored - defaults to directory "deepspeech/checkpoints" within user\'s data home specified by the XDG Base Directory Specification')
tf.app.flags.DEFINE_integer ('checkpoint_secs',  1200,         'checkpoint saving interval in seconds')
tf.app.flags.DEFINE_integer ('max_to_keep',      3,           'number of checkpoint files to keep - default value is 5')

# Exporting

tf.app.flags.DEFINE_string  ('export_dir',       '/home/zhouxinpei/deepspeech/DeepSpeech-0.1.1/data/chinese/test_linear_new_20190213/models_0.12_v2/',          'directory in which exported models are stored - if omitted, the model won\'t get exported')
tf.app.flags.DEFINE_integer ('export_version',   1,           'version number of the exported model')
tf.app.flags.DEFINE_boolean ('remove_export',    True,       'whether to remove old exported models')
tf.app.flags.DEFINE_boolean ('use_seq_length',   True,        'have sequence_length in the exported graph (will make tfcompile unhappy)')

# Reporting

tf.app.flags.DEFINE_integer ('log_level',        0,           'log level for console logs - 0: INFO, 1: WARN, 2: ERROR, 3: FATAL')
...
tf.app.flags.DEFINE_string  ('summary_dir',      'data/chinese/test/summaries/',          'target directory for TensorBoard summaries - defaults to directory "deepspeech/summaries" within user\'s data home specified by the XDG Base Directory Specification')
...
tf.app.flags.DEFINE_integer ('rnn_size',         2048,        'layer width of rnn structure')
...
tf.app.flags.DEFINE_string  ('alphabet_config_path', '/serving/data/ctc_dependence_files/bin/vocab_4500h_all_data_reverse.txt', 'path to the configuration file specifying the alphabet used by the network. See the comment in data/alphabet.txt for a description of the format.')
tf.app.flags.DEFINE_string  ('utf8_config_path',     '/serving/data/ctc_dependence_files/bin/vocab_utf8.txt', 'path to the configuration file specifying the alphabet used by network in utf-8 formattitng.')
tf.app.flags.DEFINE_string  ('lm_binary_path',       '/serving/data/ctc_dependence_files/bin/all_corpus_5gram_trie.bin', 'path to the language model binary file created with KenLM')
tf.app.flags.DEFINE_string  ('lm_trie_path',         '', 'path to the language model trie file created with native_client/generate_trie')
tf.app.flags.DEFINE_integer ('beam_width',        3,       'beam width used in the CTC decoder when building candidate transcriptions')
tf.app.flags.DEFINE_float   ('lm_weight',         1.50,       'the alpha hyperparameter of the CTC decoder. Language Model weight.')
#tf.app.flags.DEFINE_float   ('word_count_weight', 3.00,      'the beta hyperparameter of the CTC decoder. Word insertion weight (penalty).')
tf.app.flags.DEFINE_float   ('valid_word_count_weight', 3.00, 'valid word insertion weight. This is used to lessen the word insertion penalty when the inserted word is part of the vocabulary.')
tf.app.flags.DEFINE_string ('mean_std_file', '/serving/data/ctc_dependence_files/bin/mean_std_2500h_all.npz', 'mean and std file path.')
...
#tf.app.flags.DEFINE_string  ('initialize_from_frozen_model', '/home/zhouxinpei/deepspeech/DeepSpeech-0.1.1/data/chinese/test_linear_new_20181212/models_cont/output_graph.pb', 'path to frozen model to initialize from. This behaves like a checkpoint, loading the weights from the frozen model and starting training with those weights. The optimizer parameters aren\'t restored, so remember to adjust the learning rate.')
...
def initialize_globals():
    ...
    global UPDATE_OPS_COLLECTION
    UPDATE_OPS_COLLECTION = 'lstm_update_ops'
...
def BiRNN(batch_x, seq_length, is_training, dropout):
    r'''
    ...
    layer_2 = tf.reshape(layer_2, (batch_x_shape[0], -1, 41 * 32))
    layer_2_shape = tf.shape(layer_2)
    layer_2 = tf.transpose(layer_2, [1, 0, 2])
    layer_2 = tf.reshape(layer_2, (-1, 41 * 32))
    
    # 3rd layer
    b3 = variable_on_worker_level('b3', [rnn_size], tf.zeros_initializer())
    h3 = variable_on_worker_level('h3', [41 * 32, rnn_size], tf.contrib.layers.xavier_initializer())
    layer_3 = tf.add(tf.matmul(layer_2, h3), b3)
    layer_3 = tf.reshape(layer_3, [-1, batch_x_shape[0], rnn_size])
    #layer_3 = tf.minimum(layer_3, 24)
    layer_3 = batch_norm_2d(layer_3, is_training, 3)
    #print (layer_3.shape)

    gru_fw_cell_1 = tf.contrib.rnn.LSTMBlockCell(rnn_size, reuse = tf.AUTO_REUSE)
    gru_bw_cell_1 = tf.contrib.rnn.LSTMBlockCell(rnn_size, reuse = tf.AUTO_REUSE)
    rnn_outputs_1, rnn_output_states_1 = tf.nn.bidirectional_dynamic_rnn(cell_fw = gru_fw_cell_1, cell_bw = gru_bw_cell_1, inputs = layer_3, dtype = tf.float32, time_major = True)
    rnn_outputs_1 = tf.concat(rnn_outputs_1, 2)
    #rnn_outputs_1 = tf.nn.relu(rnn_outputs_1)
    b4 = variable_on_worker_level('b4', [rnn_size], tf.zeros_initializer())
    h4 = variable_on_worker_level('h4', [rnn_size*2, rnn_size], tf.contrib.layers.xavier_initializer())
    rnn_outputs_1 = tf.reshape(rnn_outputs_1, (-1, rnn_size*2))
    layer_4 = tf.add(tf.matmul(rnn_outputs_1, h4), b4)
    layer_4 = tf.reshape(layer_4, [-1, batch_x_shape[0], rnn_size])
    layer_4 = batch_norm_2d(layer_4, is_training, 4)


    gru_fw_cell_2 = tf.contrib.rnn.LSTMBlockCell(rnn_size, reuse = tf.AUTO_REUSE)
    gru_bw_cell_2 = tf.contrib.rnn.LSTMBlockCell(rnn_size, reuse = tf.AUTO_REUSE)
    rnn_outputs_2, rnn_output_states_2 = tf.nn.bidirectional_dynamic_rnn(cell_fw = gru_fw_cell_2, cell_bw = gru_bw_cell_2, inputs = layer_4, dtype = tf.float32, time_major = True)
    rnn_outputs_2 = tf.concat(rnn_outputs_2, 2)
    #rnn_outputs_2 = tf.nn.relu(rnn_outputs_2)
    b5 = variable_on_worker_level('b5', [rnn_size], tf.zeros_initializer())
    h5 = variable_on_worker_level('h5', [rnn_size*2, rnn_size], tf.contrib.layers.xavier_initializer())
    rnn_outputs_2 = tf.reshape(rnn_outputs_2, (-1, rnn_size*2))
    layer_5 = tf.add(tf.matmul(rnn_outputs_2, h5), b5)
    layer_5 = tf.reshape(layer_5, [-1, batch_x_shape[0], rnn_size])
    layer_5 = batch_norm_2d(layer_5, is_training, 5)


    gru_fw_cell_3 = tf.contrib.rnn.LSTMBlockCell(rnn_size, reuse = tf.AUTO_REUSE)
    gru_bw_cell_3 = tf.contrib.rnn.LSTMBlockCell(rnn_size, reuse = tf.AUTO_REUSE)
    rnn_outputs_3, rnn_output_states_3 = tf.nn.bidirectional_dynamic_rnn(cell_fw = gru_fw_cell_3, cell_bw = gru_bw_cell_3, inputs = layer_5, dtype = tf.float32, time_major = True)
    rnn_outputs_3 = tf.concat(rnn_outputs_3, 2)
    #rnn_outputs_2 = tf.nn.relu(rnn_outputs_2)
    b6 = variable_on_worker_level('b6', [n_hidden_6], tf.zeros_initializer())
    h6 = variable_on_worker_level('h6', [rnn_size*2, n_hidden_6], tf.contrib.layers.xavier_initializer())
    rnn_outputs_3 = tf.reshape(rnn_outputs_3, (-1, rnn_size*2))
    layer_6 = tf.add(tf.matmul(rnn_outputs_3, h6), b6)
    layer_6 = tf.reshape(layer_6, [layer_2_shape[1], batch_x_shape[0], n_hidden_6], name="logits")
    # Output shape: [n_steps, batch_size, n_hidden_6]
    return layer_6

def decode_with_lm(inputs, sequence_length, beam_width=100, top_paths=1, merge_repeated=True):
    decoded_ixs, decoded_vals, decoded_shapes, log_probabilities = (custom_op_module.ctc_beam_search_decoder_with_lm(inputs, sequence_length, beam_width=beam_width, model_path=FLAGS.lm_binary_path, trie_path=FLAGS.lm_trie_path, alphabet_path=FLAGS.alphabet_config_path, lm_weight=FLAGS.lm_weight, valid_word_count_weight=FLAGS.valid_word_count_weight, top_paths=top_paths, merge_repeated=merge_repeated))

    return ([tf.SparseTensor(ix, val, shape) for (ix, val, shape) in zip(decoded_ixs, decoded_vals, decoded_shapes)], log_probabilities)

def create_hash_table(utf8_filepath, decoded, session):
    #session = tf.Session()
    utf8_file = codecs.open(utf8_filepath, 'r')
    utf = utf8_file.readlines()
    idxs, utfs = [], []
    for i, c in enumerate(utf):
        #idxs.append(i)
        utfs.append(c.rstrip())
    #keys = tf.constant(np.array(idxs).astype(np.int64))
    values = tf.constant(np.array(utfs))
    utf8 = tf.contrib.lookup.index_to_string(decoded, mapping = values, default_value='0000')
    tf.tables_initializer().run(session=session)
    #result = table.lookup(decoded)
    #return utf8.eval()
    return utf8


def truncate_and_concatenate(tensor, stride, window):
    #stride = tf.constant(160, tf.int32)
    shape = tf.shape(tensor)[1]
    batch_size = tf.shape(tensor)[0]
    time_stamp_temp = tf.cast(tf.divide(tf.subtract(shape, window), stride), tf.int32)
    time_stamp = tf.add(1, time_stamp_temp)
    last_start_ind = tf.multiply(stride, time_stamp_temp)
    last_end_ind = tf.add(window, last_start_ind)
    sliced = tf.slice(tensor, [0, 0], [batch_size, last_end_ind])
    concatenated = tf.expand_dims(tf.slice(tensor, [0, 0], [batch_size, window]), 0)
    i = tf.constant(160, tf.int32)
    while_condition = lambda concatenated, i: tf.less(i, last_start_ind + 1)
    def body(concatenated, i):
        sliced_temp = tf.expand_dims(tf.slice(tensor, [0, i], [batch_size, window]), 0)
        concatenated = tf.concat([concatenated, sliced_temp], 0)
        return [concatenated, tf.add(i, stride)]
    result = tf.while_loop(while_condition, body, loop_vars = [concatenated, i], shape_invariants = [tf.TensorShape([None, None, 320]), i.get_shape()])
    result[0] = tf.transpose(result[0], [1,0,2])
    return result[0]


def fft(input_signal, stride_size, window_size):
    #x = tf.placeholder(dtype=tf.float32, shape=(320, None))
    truncated = truncate_and_concatenate(input_signal, stride_size, window_size)
    truncated = tf.cast(truncated, tf.float32)
    fft_dims = truncated.shape[2]
    #print (truncated.get_shape(), truncated.get_shape()[1])
    hanning = tf.contrib.signal.hann_window(fft_dims, periodic=False, dtype=tf.float32, name=None)
    hanning = tf.expand_dims(tf.expand_dims(hanning, 0), 0)
    temp = tf.multiply(truncated, hanning)
    fft = tf.square(tf.abs(tf.spectral.rfft(temp, fft_length=[fft_dims])))
    scale = tf.divide(2.0, tf.multiply(16000.0, tf.reduce_sum(tf.square(hanning))))
    t = tf.fill((fft.shape[2]-2, 1), 1.0)
    t1 = tf.constant(0.5, shape=(1,1))
    t = tf.concat([t1, t, t1], axis=0)
    t = tf.expand_dims(tf.transpose(t, [1,0]), 0)
    fft = tf.multiply(fft, t)
    eps = tf.constant(1e-14, dtype=tf.float32)
    fft_change = tf.add(tf.multiply(fft, scale), eps)
    #fft_final = tf.transpose(tf.math.log(fft_final), [0,2,1])

    data = np.load(FLAGS.mean_std_file)
    mean = tf.expand_dims(tf.transpose(tf.convert_to_tensor(data['mean'], dtype=tf.float32, name='mean'), [1,0]), 0)
    std = tf.expand_dims(tf.transpose(tf.convert_to_tensor(data['std'], dtype=tf.float32, name='std'), [1,0]), 0)
    print (fft_change.shape, mean.shape, std.shape)
    fft_final = tf.divide(tf.subtract(tf.math.log(fft_change), mean), std, name='fft_final1')
    #fft_final = tf.transpose(tf.expand_dims(fft_final, [0,2,1], name='fft_final2')
...
def calculate_mean_edit_distance_and_loss(model_feeder, tower, dropout):
    ...
    logits = BiRNN(batch_x, tf.to_int64(batch_seq_len), True, dropout)
    ...
    if FLAGS.use_warpctc:
        ...
        #print (logits.shape)
        #print (batch_seq_len)
        batch_seq_len = tf.ceil(batch_seq_len/3)
        batch_seq_len = tf.cast(batch_seq_len, dtype = tf.int32)
    ...
    #decoded, _ = decode_with_lm(logits, batch_seq_len, FLAGS.beam_width, 1, False, FLAGS.lm_binary_path, FLAGS.lm_trie_path, FLAGS.alphabet_config_path, FLAGS.lm_weight, FLAGS.valid_word_count_weight)
...
def create_optimizer(global_step):
    boundaries = [20000, 40000, 60000]
    values = [FLAGS.learning_rate, FLAGS.learning_rate * 0.2, FLAGS.learning_rate * 0.1, FLAGS.learning_rate * 0.02]
...
def train(server=None):
    ...
    optimizer = create_optimizer(global_step)
    ...
    apply_train_op = tf.group(apply_gradient_op, batchnorm_updates_op)
    print ('Batchnorm opearations has been registered')
        ...
        with tf.train.MonitoredTrainingSession(master='' if server is None else server.target,
            ...
            try:
                if is_chief:
                    ...
                    steps_per_epoch = COORD.start_coordination(model_feeder, step)
                ...
                while job and not session.should_stop():
                    ...
                    #train_op = tf.group(apply_gradient_op, batchnorm_updates_op) if job.set_name == 'train' else []
                    train_op = apply_train_op if job.set_name == 'train' else []
                    ...
                    for job_step in range(job.steps):
                        ...
                        # Add batch to loss
                        current_epoch_step = current_step % steps_per_epoch
                        ...
                        if current_epoch_step % 100 == 0:
                            #print (print_loss, current_step, current_epoch_step, current_epoch, batch_loss)
                            print ('Current Average loss is %f after %d steps in epoch %d' % (print_loss/current_epoch_step, current_epoch_step, current_epoch)) 
                            print ('Learning rate: %f' % (session.run(optimizer._lr)))
                        if current_step % steps_per_epoch == 0:
                            print_loss = 0.0
...
def create_inference_graph(batch_size=None, output_is_logits=False, use_new_decoder=False):
    ...
    #input_tensor = tf.placeholder(tf.float32, [batch_size, None, 161], name='input_node')
    stride_size = tf.constant(160, tf.int32)
    window_size = tf.constant(320, tf.int32)
    input_tensor = tf.placeholder(tf.float32, [None, None], name='input_node')
    is_training = tf.placeholder(tf.bool, name = 'training_phase')
    # Calculate the logits of the batch using BiRNN
    fft_result = fft(input_tensor, stride_size, window_size) 
    logits = BiRNN(fft_result, None, is_training, no_dropout)
    print (logits.shape)
    if output_is_logits:
        return logits

    # Beam search decode the batch
    seq_length = tf.fill(tf.shape(input_tensor)[0:1], tf.shape(logits)[0])
    ...
    decoded = tf.convert_to_tensor([tf.sparse_tensor_to_dense(sparse_tensor) for sparse_tensor in decoded], name='output_node')
...
def export():
    ...
    with tf.device('/cpu:0'):
        ...
        inputs, outputs = create_inference_graph(use_new_decoder = True)
        print (inputs)
        ...
        try:
            ...
            tf.train.write_graph(session.graph, FLAGS.export_dir, input_graph_name, as_text=True)
...
def do_single_file_inference(input_file_path):
    with tf.Session(config=session_config) as session:
        ...
        mfcc = audiofile_to_linear_input_vector(input_file_path, n_input, n_context)
    ...
    if FLAGS.train or FLAGS.test:
        if len(FLAGS.worker_hosts) == 0:
            ...
            if FLAGS.job_name == 'ps':
                # We are a parameter server and therefore w:qe just wait for all workers to finish
