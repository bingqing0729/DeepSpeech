...
def load_vocab():
    import pickle
    if hp.isqwerty:
        return pickle.load(open('pinyin/data/vocab.qwerty.pkl', 'rb'))
    else:
        return pickle.load(open('pinyin/data/vocab.nine.pkl', 'rb'))
...
def load_train_data():
    ...
    with codecs.open('t', 'w', 'utf-8') as fout:
        for line in codecs.open('pinyin/data/zh.tsv', 'r', 'utf-8'):
            try:
